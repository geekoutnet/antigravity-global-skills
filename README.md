...
| :--- | :--- | :--- |
| **🚀 超级后端开发** | 后端专家 | **触发**: `backend`, `Java`, `Go`, `Redis`<br>**能力**: 精通 Java/Go/Python/Node.js/Rust，高并发架构设计，分布式系统解决方案。 |
| **✨ 超级前端开发** | 前端专家 | **触发**: `frontend`, `React`, `Vue`, `动画`<br>**能力**: 极致性能优化 (FPS/LCP)，复杂交互实现，WebGL/Three.js 动效，像素级 UI 还原。 |
| **📐 API设计师** | 接口架构师 | **触发**: `API`, `RESTful`, `GraphQL`, `gRPC`<br>**能力**: RESTful 规范设计，GraphQL/gRPC 选型，统一响应格式，API 版本管理与治理。 |
...
| Skill 名称 | 角色定位 | 核心超能力与触发词 |
| :--- | :--- | :--- |
| **🏛️ 全栈架构师** | 架构顾问 | **触发**: `全栈`, `系统设计`, `K8s`, `技术选型`<br>**能力**: 前后端 + 基础设施全局规划，技术选型，微服务拆分，技术债务治理。 |
| **🏗️ 系统架构师** | 脚手架专家 | **触发**: `架构`, `structure`, `设计模式`<br>**能力**: 项目目录结构规划，设计模式建议，技术栈选型与初始化。 |
| **🔄 重构大师** | 代码治理者 | **触发**: `重构`, `refactor`, `代码异味`, `SOLID`<br>**能力**: 代码异味检测，SOLID 原则驱动重构，绞杀模式迁移，安全渐进式改造。 |

### 🛡️ 质量与安全

| Skill 名称 | 角色定位 | 核心超能力与触发词 |
| :--- | :--- | :--- |
| **🛡️ 安全卫士** | 安全专家 | **触发**: `安全`, `漏洞`, `XSS`, `OWASP`<br>**能力**: OWASP Top 10 防御，认证授权安全，供应链审计，代码级安全扫描。 |
...
| Skill 名称 | 角色定位 | 核心超能力与触发词 |
| :--- | :--- | :--- |
| **⚡ 性能优化师** | 性能调优师 | **触发**: `性能`, `慢查询`, `Lighthouse`, `压测`<br>**能力**: 前端 Core Web Vitals 优化，后端 CPU/IO 调优，数据库慢查询，全链路分析。 |
| **🐳 DevOps工程师** | 运维自动化 | **触发**: `Docker`, `K8s`, `CI/CD`, `Nginx`<br>**能力**: 容器化部署，K8s 编排，CI/CD 流水线，IaC，Nginx 负载均衡，可观测性。 |
| **🏆 超级Git管理大师** | 仓库管理员 | **触发**: `git`, `commit`, `push`, `pull`<br>**能力**: 自动诊断仓库状态，规范提交信息，安全推送，智能冲突建议。 |
...
| **🤖 AI提示词工程师** | AI应用专家 | **触发**: `prompt`, `AI`, `RAG`, `Agent`<br>**能力**: 提示词工程 (CoT/ToT/ReAct)，RAG 系统设计，Agent 编排，Function Calling。 |
